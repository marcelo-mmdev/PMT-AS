...
export default function PessoasPage() {
  ...
  const handleAddPessoa = (novaPessoa: Pessoa) => {
    setPessoas((prev) => [...prev, { ...novaPessoa, id: String(Date.now()) }])
    setAbrirAdd(false)
  }
  ...
  const handleDownloadPDF = (pessoa: Pessoa) => {
    ...
    doc.rect(22, 45, 28, 36) // 3x4 aproximado em mm
    doc.setFontSize(8)
    ...
    // QRCode (se disponível no canvas)
    const canvas = qrRef.current
    if (canvas) {
      const dataUrl = canvas.toDataURL("image/png")
      // insere no canto inferior direito da moldura
      ...
      <div className="flex h-screen">
        ...
        <DataTable
          columns={getColumns({
            ...
            onDelete: (id) => setPessoas((prev) => prev.filter((p) => p.id !== id)),
          ...
          <Dialog open={!!editPessoa} onOpenChange={() => setEditPessoa(null)}>
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription className="sr-only">
                  Altere os campos e salve para atualizar os dados da pessoa.
                </DialogDescription>
              </DialogHeader>
          ...
          <Dialog open={!!carteirinhaPessoa} onOpenChange={() => setCarteirinhaPessoa(null)}>
            <DialogContent className="max-w-lg">
              <DialogHeader>
                ...
                <DialogDescription className="sr-only">
                  Visualização da carteirinha com QR Code e opção de download em PDF.
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="flex justify-end pt-3">
                <Button onClick={() => handleDownloadPDF(carteirinhaPessoa)}>
                  Baixar PDF
                </Button>
              </div>
            </DialogContent>
